...
const programTypeOptions: { value: string; label: string }[] = [
...
const TimetablesTab = () => {
  ...
  useEffect(() => {
    const fetchTimetables = async () => {
      try {
        ...
        setFilteredTimetables(sortedData);
      } catch (error) {
        const errorMessage =
          (error as Error)?.message || "Failed to fetch timetables";
  ...
  useEffect(() => {
    const filtered = timetables.filter((timetable) => {
      const matchesSearch = timetable.program
        .toLowerCase()
        .startsWith(searchTerm.toLowerCase());
      const matchesType =
        typeFilter === "all" || timetable.program_type === typeFilter;
      return matchesSearch && matchesType;
    });

    setFilteredTimetables(filtered);
    setCurrentPage(1); // Reset to first page whenever filters change
  }, [timetables, searchTerm, typeFilter]);

  // Calculate pagination
  const indexOfLastItem = currentPage * itemsPerPage;
  const indexOfFirstItem = indexOfLastItem - itemsPerPage;
  const currentItems = filteredTimetables.slice(
    indexOfFirstItem,
    indexOfLastItem,
  );
  ...
  const handleDeleteTimetable = async () => {
    try {
      ...
      await deleteTimetable(currentTimetable.id);
      const res = timetables.filter((t) => t.id !== currentTimetable.id);
      setFilteredTimetables(res);
      setTimetables(res);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setModalLoading(true);
    try {
      let updatedTimetables: Timetable[];
      if (editMode) {
        const data = await updateTimetable(currentTimetable);
        updatedTimetables = timetables
          .map((t) => (t.id === currentTimetable.id ? data : t))
          .sort(
            (a, b) =>
              new Date(b.last_updated).getTime() -
              new Date(a.last_updated).getTime(),
        ...
        const data = await postTimetable(currentTimetable);
        updatedTimetables = [...timetables, data].sort(
          (a, b) =>
            new Date(b.last_updated).getTime() -
            new Date(a.last_updated).getTime(),
        );
      ...
      setFilteredTimetables(updatedTimetables);
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">
            Manage course timetables and schedules.
          </p>
        </div>
        <motion.button
      ...
      {isLoading ? (
        ...
        >
          ...
          <p className="text-gray-500 mt-2 max-w-xs mx-auto">
            We couldn't find any timetables matching your search or filters.
          </p>
          <button
            onClick={() => {
              setSearchTerm("");
        ...
        <>
                  ...
                  {currentItems.map((timetable) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex justify-end gap-2">
                          <button
                            onClick={() => handleViewPdf(timetable.file_link)}
          ...
          <div className="flex flex-col sm:flex-row justify-between items-center bg-white p-4 rounded-2xl border border-gray-100 gap-4 mt-6">
            <p className="text-sm text-gray-500 font-medium italic">
              ...
              {Math.min(indexOfLastItem, filteredTimetables.length)} of{" "}
      ...
      <AnimatePresence>
        ...
        {warningModal && (
          <WarningModal
            ...
            handleDelete={handleDeleteTimetable}
